...
| `get-household-status` | `profile:read` | — | Members and roles, who's on duty, the children, the latest shift, the active timer with elapsed time |
...
| `get-timer` | `timer:read` | — | The running timer (or none) with elapsed time |
| `start-timer` | `timer:write` | `type` (`nurse`/`pump`/`sleep`), `baby_id?` | Starts (replaces) the household's single timer |
| `stop-timer` | `timer:write` (+ `entries:write` to log) | `log?` (bool, default `true`), `detail?` | Stops the timer; with `log: true` it also writes the matching entry — sleep logs the elapsed minutes as `detail`, nurse/pump log the `detail` you pass (side / ounces). The convenience the raw [REST timer](integrations.md#delete-v1timer--timerwrite) leaves to the caller |
...
One resource, `household-overview` (`profile:read` + `entries:read`): a markdown snapshot of the household — children with birthdates, members, who's on duty, the running timer, and today's headline numbers. Useful as ambient context for clients that attach resources automatically. No prompts in v1.
